# Binary Search Tree
# left side smaller than root, right greater than root
# balanced bst == height diff <= 1
...
class BST:
	...
	# end = ' ' is to print all the values on one line with spacing instead of \n
    ...
    def minimun(self):
        if self.left is None:
            print("Mininum is ", self.data)
        else:
            self.left.minimun()

    def maximun(self):
        if self.right is None:
            print("Maximun is ", self.data)
        else:
            self.right.maximun()
